...
// Allow trip_id from GET (when opening the form) or POST (when submitted)
$trip_id = isset($_GET['trip_id']) ? intval($_GET['trip_id']) : null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $category_name = isset($_POST['category_name']) ? trim($_POST['category_name']) : '';
    $allocation_amount = isset($_POST['allocation_amount']) ? floatval($_POST['allocation_amount']) : 0.00;
    $trip_id = isset($_POST['trip_id']) ? intval($_POST['trip_id']) : $trip_id;

    if (empty($category_name) || empty($trip_id)) {
        $error_message = 'Category name and trip ID are required.';
    } else {
...
<main>
    <section class="createTrip-section">
        <h1>Add Category</h1>
        <p>Create a new budget category for this trip.</p>
        ...
        <form action="add_category.php?trip_id=<?php echo htmlspecialchars($trip_id); ?>" method="POST">
            ...
            <label for="allocation_amount">Allocation Amount (<?php echo $symbol; ?>)</label>
            <input type="number" id="allocation_amount" name="allocation_amount" step="0.01" min="0" value="0.00" required>
