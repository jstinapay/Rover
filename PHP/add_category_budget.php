...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$budget_data) {
        ...
        if (($current_allocated + $allocation_amount) > $total_budget) {
            ...
            $sql_insert = "INSERT INTO category_budget (trip_id, category_id, allocated_budget) 
                           VALUES (?, ?, ?)";
            $stmt_insert = $conn->prepare($sql_insert);
            $stmt_insert->bind_param("iid", $trip_id, $category_id, $allocation_amount);
            ...
            if ($stmt_insert->execute()) {
                ...
                $error_message = 'Execute failed: ' . $stmt_insert->error;
                $stmt_insert->close();
            }
        }
    }
...
$sql_available_cats = "SELECT c.category_id, c.category_name
                       FROM category c
...
$stmt_cats = $conn->prepare($sql_available_cats);
$stmt_cats->bind_param("i", $trip_id);
$stmt_cats->execute();
$available_categories = $stmt_cats->get_result()->fetch_all(MYSQLI_ASSOC);
$stmt_cats->close();
...
<main>
    <section class="createTrip-section">
        ...
        <p>Add a new budget from your available categories.</p>

        <?php if (!empty($error_message)): ?>
            <div class="error-message"><?php echo htmlspecialchars($error_message); ?></div>
        <?php endif; ?>

        <form action="add_category_budget.php?trip_id=<?php echo htmlspecialchars($trip_id); ?>" method="POST">
            <input type="hidden" name="trip_id" value="<?php echo htmlspecialchars($trip_id); ?>">

            <label for="category_id">Category Name</label>
            <select id="category_id" name="category_id" required>
                <?php if (empty($available_categories)): ?>
                    <option value="" disabled>All categories have been added.</option>
                <?php else: ?>
            ...
            <div class="input-wrapper">
                <input type="number" min="0" step=".01" id="allocation_amount" name="allocation_amount" placeholder="0.00" required>
            </div>

            <button type="submit" class="submit-btn" <?php if (empty($available_categories)) echo 'disabled'; ?>>
                Add Category
            </button>
