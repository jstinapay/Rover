...
$sql_expenses = "SELECT
                 ...
                 WHERE cb.trip_id = ? AND cb.category_budget_id = ?
                 ORDER BY e.expense_date DESC, e.expense_id DESC";
$stmt_expenses = $conn->prepare($sql_expenses);
$stmt_expenses->bind_param("ii", $trip_id, $category_budget_id);
...
if (!empty($expenses)) {
    ...
    $sql_cat = "SELECT c.category_name
                FROM category_budget cb
                JOIN category c ON cb.category_id = c.category_id
                WHERE cb.category_budget_id = ?
                LIMIT 1";
    $stmt_cat = $conn->prepare($sql_cat);
    $stmt_cat->bind_param("i", $category_budget_id);
    $stmt_cat->execute();
    $res_cat = $stmt_cat->get_result()->fetch_assoc();
    $stmt_cat->close();
    if ($res_cat && isset($res_cat['category_name'])) {
        $category_name = $res_cat['category_name'];
    }
}
?>
<!DOCTYPE html>
...
<main>
    <section class='dashboard'>
        <h1><?php echo htmlspecialchars($category_name ?? 'Category'); ?> Expenses</h1>
    </section>
