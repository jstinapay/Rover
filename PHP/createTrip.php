...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        $sql_cat_types = "SELECT category_id FROM category";
        $cat_types_result = $conn->query($sql_cat_types);

        $sql_budget = "INSERT INTO category_budget (trip_id, category_id, allocated_budget) VALUES (?, ?, 0.00)";
        $stmt_budget = $conn->prepare($sql_budget);

        while ($row = $cat_types_result->fetch_assoc()) {
            $category_id = $row['category_id'];
            $stmt_budget->bind_param("ii", $new_trip_id, $category_id);
            $stmt_budget->execute();
        }
        $stmt_budget->close();
